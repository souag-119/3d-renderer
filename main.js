...
(function(){
  ...
  container.addEventListener('pointermove', (e)=>{
    ...
    if(pointers.size===2 && dragMode==='twoFinger'){
      ...
      // تحرك أمامي/خلفي حقيقي (وليس فقط تصغير حول هدف ثابت): نحرّك كلاً من target وdistance معاً
      // بحيث يبدو التأثير كتقدم فعلي داخل البيئة، دون حدود قصوى للاقتراب أو الابتعاد
      ...
      scene.camera.distance = Math.max(0.05, twoFingerStart.camDistance - distDelta*0.02);
  ...
  function computeCamBasis(camObjRef, roll){
    // نستخدم نفس معادلة الدوران المستخدمة في التصميم البصري للكاميرا (getForwardDirection)
    // لضمان تطابق منظور الدخول الفعلي مع الاتجاه الظاهر بصرياً في المشهد تماماً
    ...
    // تطبيق roll: دوران right/up حول forward (تحكم "المقود" بإصبعين)
  ...
  function enterSceneCamera(camObjRef){
    ...
    // yaw/pitch تُستخدمان الآن فقط كزوايا تحكم حرة إضافية أثناء التجول داخل الكاميرا
    // (تُضاف لدوران الكائن الأصلي وليست بديلاً عنه) لضمان توافق الاتجاه الابتدائي مع التصميم البصري
  ...
  function getActiveRenderCamera(){
    ...
    // ابنِ كائناً مؤقتاً بنفس دوران الكاميرا الأصلي + التحكم الحر الإضافي (yawDelta/pitchDelta) من السحب
    const tempObj = {
      rotation: [
