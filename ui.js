// ======================================================================
// ui.js — منطق واجهة المستخدم (لوحات، تبويبات، خصائص، مواد، بيئة، رندر)
// ======================================================================
...
const UI = {
  ...
  cacheEls() {
    const ids = [
      'panelScene', 'panelProps', 'btnMenuToggleLeft', 'btnMenuToggleRight',
      ...
      'lightRange', 'lightRangeNum', 'lightSpotAngle', 'lightSpotAngleNum',
      'lightSpotBlend', 'lightSpotBlendNum',
      'objCamFov', 'objCamFovNum', 'objCamDof', 'objCamFocus', 'objCamFocusVal',
      'objCamAperture', 'objCamApertureVal', 'btnUseThisCamera',
      ...
      'btnTexColor', 'btnTexNormal', 'btnTexRoughness',
      'texColorInput', 'texNormalInput', 'texRoughnessInput', 'texStatusRow',
      'envColorTop', 'envColorBottom', 'envIntensity', 'envIntensityVal',
      'realtimeToggle', 'bounces', 'bouncesVal', 'denoiserEnabled',
      'denoiserStrength', 'denoiserStrengthVal',
      'bloomEnabled', 'bloomStrength', 'bloomStrengthVal',
      'bloomThreshold', 'bloomThresholdVal',
  ...
  // ---- ربط التبويبات المصغرة (إضافة / شجرة) ----
  bindMiniTabs() {
  ...
  // ---- ربط التبويبات الرئيسية (عنصر / مادة / بيئة / رندر) ----
  bindTabs() {
  ...
  // ---- ربط إضافة الكائنات (مجسمات، إضاءات، كاميرا) ----
  bindSceneAdd() {
    // إضافة مجسمات
  ...
  refreshSceneTree() {
    ...
    this.scene.objects.forEach(o => {
      ...
      li.innerHTML = `<span class="obj-icon">${icon}</span>
                      ...
                      <span class="vis-toggle ${o.visible ? '' : 'hidden-obj'}">👁</span>`;
      li.onclick = (e) => {
  ...
  // ---- أداة ربط شريط تمرير بحقل رقمي ----
  linkSliderNum(sliderId, numId, onChange, opts = {}) {
    const slider = this.el[sliderId],
      num = this.el[numId];
    ...
    slider.oninput = () => {
      num.value = opts.digits !== undefined ? parseFloat(slider.value).toFixed(opts.digits) : slider.value;
  ...
  // ---- ربط خصائص الكائن (الموضع، الدوران، الحجم، الإضاءة، الكاميرا) ----
  bindObjectProps() {
    // الاسم
  ...
  // ---- المواد: القائمة، التحميل، التطبيق ----
  ...
  loadMaterialToForm(mat) {
    ...
    this.el.matMetallicNum.value = mat.metallic.toFixed(2);
    const roughPct = Math.round((mat.roughness || 0.5) * 100);
    this.el.matRoughness.value = roughPct;
    this.el.matRoughnessNum.value = roughPct;
    const smoothPct = Math.round((mat.smoothShade !== undefined ? mat.smoothShade : 1) * 100);
    this.el.matSmoothShade.value = smoothPct;
    this.el.matSmoothShadeVal.textContent = smoothPct;
    this.el.matSubsurface.value = mat.subsurface || 0;
    this.el.matSubsurfaceNum.value = (mat.subsurface || 0).toFixed(2);
    this.el.matEmission.value = mat.emission || 0;
    this.el.matEmissionNum.value = (mat.emission || 0).toFixed(2);
    ...
    this.el.matIOR.value = mat.ior || 1.45;
    this.el.matIORNum.value = (mat.ior || 1.45).toFixed(2);
    this.el.matMetallicT.value = mat.metallic || 0;
    this.el.matMetallicTNum.value = (mat.metallic || 0).toFixed(2);
    const roughPctT = Math.round((mat.roughness !== undefined ? mat.roughness : 0.05) * 100);
    this.el.matRoughnessT.value = roughPctT;
    this.el.matRoughnessTNum.value = roughPctT;
    ...
    // بقعة
    this.el.matMetallicM.value = mat.metallic || 0;
    this.el.matMetallicMNum.value = (mat.metallic || 0).toFixed(2);
    const roughPctM = Math.round((mat.roughness || 0.5) * 100);
    this.el.matRoughnessM.value = roughPctM;
    this.el.matRoughnessMNum.value = roughPctM;
    this.el.matEmissionM.value = mat.emission || 0;
    this.el.matEmissionMNum.value = (mat.emission || 0).toFixed(2);
  ...
  // ---- قراءة قيم المادة من النموذج (للمعاينة والتطبيق) ----
  ...
  bindMaterialProps() {
    ...
    // شرائح المتريال (تحديث المعاينة الحية)
    const liveFields = [
      ['matMetallic', 'matMetallicNum', 2],
      ['matRoughness', 'matRoughnessNum', 0],
      ['matSmoothShade', null, 0],
      ['matSubsurface', 'matSubsurfaceNum', 2],
      ['matEmission', 'matEmissionNum', 2],
      ['matTransmission', 'matTransmissionNum', 2],
      ['matIOR', 'matIORNum', 2],
      ['matMetallicT', 'matMetallicTNum', 2],
      ['matRoughnessT', 'matRoughnessTNum', 0],
      ['matVolDensity', 'matVolDensityNum', 2],
      ['matVolScatter', 'matVolScatterNum', 2],
      ['matMetallicM', 'matMetallicMNum', 2],
      ['matRoughnessM', 'matRoughnessMNum', 0],
      ['matEmissionM', 'matEmissionMNum', 2],
      ['matMaskSoftness', 'matMaskSoftnessNum', 2]
    ];
    ...
    this.el.btnApplyMaterial.onclick = () => {
      ...
      if (o && o.kind === 'mesh') {
        o.material = mat;
        // إعادة تعيين رفع الميش لتحديث المادة
        o._meshUploaded = false;
      }
      this.refreshMaterialSelect();
      this.el.materialSelect.value = mat.id;
      this.pushUndo();
  ...
  // ---- معاينة المادة (في تبويب المتريال) ----
  initMaterialPreview() {
    const canvas = this.el.matPreviewCanvas;
    try {
      // نستخدم محركاً مستقلاً للمعاينة
  ...
  _buildPreviewScene() {
    ...
    sphereObj.scaleXYZ = [0.8, 0.8, 0.8];
    // إنشاء مادة افتراضية
    const mat = new Material('معاينة');
    sphereObj.material = mat;
  ...
  updateMaterialPreviewLive(matValues) {
    ...
    const obj = this.matPreviewScene._sphereObj;
    if (!obj) return;
    // نسخ القيم إلى مادة الكرة
    Object.assign(obj.material, matValues);
    // إعادة تعيين رفع الميش
    obj._meshUploaded = false;
  },

  _runMatPreviewLoop() {
    const step = () => {
      requestAnimationFrame(step);
      if (!this.matPreviewEngine || !this.matPreviewScene) return;
      // معاينة فورية بدون تراكم
