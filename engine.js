...
vec3 subsurfaceLighting(vec3 pos, vec3 nrm, vec3 albedo, float sssAmount, int objId){
  vec3 result = vec3(0.0);
  vec3 backNrm = -nrm;
  for(int i=0;i<MAX_LIGHTS;i++){
    if(i>=uLightCount) break;
    ...
    if(backNdl<=0.0) continue; // يظهر فقط في الجهة المقابلة لمصدر الضوء (خلف الإشعاع)

    // قياس السماكة الفعلية عبر إطلاق شعاع من نقطة السطح باتجاه الضوء من الخلف
    float thickness = exitDistance(pos - backNrm*0.001, ldir, objId);
    // العتبة المطلوبة لنفوذ الضوء تكبر مع زيادة sssAmount (مادة أكثر كثافة تحتاج سماكة أقل لتنفذ عند sss عالي)
    float requiredThinness = mix(0.05, 1.2, sssAmount);
    float penetration = exp(-thickness/max(requiredThinness,0.001));

    vec3 sssColor = albedo; // اللون يتبع لون المادة نفسها
    result += sssColor * radiance * backNdl * penetration * sssAmount;
  }
  return result;
}
...
vec3 tracePath(vec3 ro, vec3 rd){
  ...
  for(int bounce=0; bounce<12; bounce++){
    ...
    if(mType==3){
      Hit behind = sceneIntersect(h.pos + rd*0.004, rd, h.id);
      float proximity = behind.id>=0 ? (behind.t) : 1e9;
      if(proximity > uMatMaskSoftness[h.id]*0.5+0.02){
        ro = h.pos + rd*0.004;
        continue;
      }
      vec3 albedoM = uMatColor[h.id];
      float edgeFade = 1.0 - clamp(proximity/max(uMatMaskSoftness[h.id]*0.5+0.02,0.001),0.0,1.0);
      vec3 n = h.nrm;
      ...
      radiance += throughput*(uMatEmission[h.id]+direct)*edgeFade;
      ro = h.pos + rd*0.004;
      continue;
    }
    ...
    if(mType==2){
      float density = uMatVolumeDensity[h.id];
      float scatter = uMatVolumeScatter[h.id];
      vec3 direct = directLighting(h.pos,nrm,viewDir,albedo,0.0,0.9,h.id);
      radiance += throughput * direct * scatter * (1.0-density);
      throughput *= mix(albedo, vec3(1.0), 1.0-density);
      rd = mix(cosineSampleHemisphere(nrm), rd, 1.0-scatter);
      ro = h.pos + rd*0.004;
      if(bounce>3){
    ...
    if(subsurface>0.01){
      radiance += throughput * subsurfaceLighting(h.pos,nrm,albedo,subsurface,h.id);
    }
...
void main(){
  ...
  // أوزان غاوسية ثابتة (9 عينات) لبلور ناعم وخفيف الحساب
  float weights[5];
...
class PathTracerEngine{
  ...
  resize(w,h){
    ...
    // أهداف Bloom بدقة أقل (نصف الدقة) لأداء أفضل وبلور أنعم طبيعياً
    if(this.bloomA) gl.deleteFramebuffer(this.bloomA.fbo);
    if(this.bloomB) gl.deleteFramebuffer(this.bloomB.fbo);
    if(this.bloomA) gl.deleteTexture(this.bloomA.tex);
    if(this.bloomB) gl.deleteTexture(this.bloomB.tex);
    const bw = Math.max(4,Math.floor(w/2)), bh = Math.max(4,Math.floor(h/2));
    this.bloomW = bw; this.bloomH = bh;
    this.bloomA = this.makeFloatTarget(bw,bh);
    this.bloomB = this.makeFloatTarget(bw,bh);
  ...
  computeBloom(bloomThreshold, bloomRadius){
    const gl = this.gl;
    // 1) استخراج المناطق الساطعة من tex0 إلى bloomA بدقة أقل
    ...
    // 2) بلور غاوسي فصلي متكرر (أفقي ثم رأسي) عدة مرات لنعومة أكبر
    const passes = 3;
    for(let p=0;p<passes;p++){
      ...
      gl.uniform2f(gl.getUniformLocation(this.progBlur,'uResolution'), this.bloomW, this.bloomH);
  ...
  present(denoise, denoiseStrength, bloom, bloomStrength, bloomThreshold){
    ...
    if(bloom){
      this.computeBloom(bloomThreshold, 1.6);
    }
    ...
    gl.bindTexture(gl.TEXTURE_2D, bloom ? this.bloomA.tex : this.tex0);
