...
// يحتوي على: Path Tracer (للرندر النهائي والوضع الحي)، و Rasterizer خفيف
// (للمعاينة السريعة قبل الرندر)، و Bloom pass.
// ======================================================================
...
uniform vec3 uMatSubsurfaceColor[MAX_OBJS];
...
uniform float uLightSpotCos[MAX_LIGHTS];
uniform float uLightSpotBlendCos[MAX_LIGHTS];
...
bool sceneOcclude(vec3 ro, vec3 rd, float maxT){
  for(int i=0;i<MAX_OBJS;i++){
    ...
    if(uMatTransmission[i] > 0.85) continue;
    vec3 lro = uObjRotInv[i]*(ro-uObjPos[i]);
    vec3 lrd = uObjRotInv[i]*rd;
    int type = uObjType[i];
    float t; vec3 nl;
...
vec3 directLighting(vec3 pos, vec3 nrm, vec3 viewDir, vec3 albedo, float metallic, float roughness){
  ...
  for(int i=0;i<MAX_LIGHTS;i++){
    ...
    if(lt==1){ // sun
      ...
      float atten = (1.0/max(dist*dist,0.01)) * rangeAtten;
      radiance = uLightColor[i]*uLightIntensity[i]*atten;

      if(lt==3){ // spot cone falloff
        vec3 spotDir = normalize(uLightDir[i]);
        float cosAngle = dot(-ldir, spotDir);
        float spotFactor = smoothstep(uLightSpotBlendCos[i], uLightSpotCos[i], cosAngle);
        radiance *= spotFactor;
      }
    }
...
// إضاءة نفوذية تقريبية (subsurface) عند سقوط الضوء من الجهة المقابلة للناظر
vec3 subsurfaceLighting(vec3 pos, vec3 nrm, vec3 viewDir, vec3 sssColor, float sssAmount){
  vec3 result = vec3(0.0);
  vec3 backNrm = -nrm;
  for(int i=0;i<MAX_LIGHTS;i++){
    if(i>=uLightCount) break;
    ...
    if(lt==1){
      ...
      vec3 toL = uLightPos[i]-pos;
      dist = length(toL);
      ldir = toL/max(dist,0.0001);
      float rangeAtten = clamp(1.0 - pow(dist/max(uLightRange[i],0.01), 4.0), 0.0, 1.0);
      radiance = uLightColor[i]*uLightIntensity[i]*(1.0/max(dist*dist,0.01))*rangeAtten;
      if(lt==3){
        vec3 spotDir = normalize(uLightDir[i]);
        float cosAngle = dot(-ldir, spotDir);
        radiance *= smoothstep(uLightSpotBlendCos[i], uLightSpotCos[i], cosAngle);
      }
    }
    float backNdl = max(dot(backNrm, ldir), 0.0);
    // ضوء يمر من الخلف عبر المادة شبه الشفافة
    float wrap = pow(backNdl, 1.5) * sssAmount;
    result += sssColor * radiance * wrap * 0.6;
  }
  return result;
}
...
vec3 tracePath(vec3 ro, vec3 rd){
  ...
  for(int bounce=0; bounce<12; bounce++){
    ...
    float subsurface = uMatSubsurface[h.id];
    vec3 sssColor = uMatSubsurfaceColor[h.id];
    ...
    if(subsurface>0.01){
      radiance += throughput * subsurfaceLighting(h.pos,nrm,viewDir,sssColor,subsurface);
    }
    ...
    if(transmission>0.01 && rr<transmission){
      ...
      // امتصاص لوني بسيط داخل الوسط الشفاف (Beer-Lambert تقريبي)
...
// ------------------------------------------------------------------
// شيدر Rasterizer خفيف للمعاينة السريعة (قبل بدء الرندر النهائي)
// يرسم كل جسم كـ SDF بسيط في شاشة كاملة مع إضاءة Lambert+Specular مبسطة
// ------------------------------------------------------------------
...
bool intersectSphere(vec3 ro, vec3 rd, vec3 c, float r, out float t){
  ...
  float t0=-b-h, t1=-b+h; t = t0>0.001?t0:t1; return t>0.001;
}
bool intersectBox(vec3 ro, vec3 rd, vec3 hs, out float t, out vec3 nl){
  vec3 invD=1.0/rd; vec3 t0s=(-hs-ro)*invD; vec3 t1s=(hs-ro)*invD;
  vec3 tsm=min(t0s,t1s); vec3 tbg=max(t0s,t1s);
  float tmin=max(max(tsm.x,tsm.y),tsm.z); float tmax=min(min(tbg.x,tbg.y),tbg.z);
  if(tmax<0.0||tmin>tmax) return false;
  t = tmin>0.001?tmin:tmax; if(t<0.001) return false;
  vec3 p=ro+rd*t; vec3 d=abs(p)/hs;
  if(d.x>d.y&&d.x>d.z) nl=vec3(sign(p.x),0,0);
  else if(d.y>d.z) nl=vec3(0,sign(p.y),0);
  else nl=vec3(0,0,sign(p.z));
  return true;
}
bool intersectPlane(vec3 ro, vec3 rd, vec2 hs, out float t){
  if(abs(rd.y)<1e-5) return false;
  t=-ro.y/rd.y; if(t<0.001) return false;
  vec3 p=ro+rd*t; if(abs(p.x)>hs.x||abs(p.z)>hs.y) return false;
  return true;
}
bool intersectCylinder(vec3 ro, vec3 rd, float r, float hh, out float t, out vec3 nl){
  float a=rd.x*rd.x+rd.z*rd.z, b=2.0*(ro.x*rd.x+ro.z*rd.z), c=ro.x*ro.x+ro.z*ro.z-r*r;
  float bestT=1e9; bool found=false; vec3 bestN;
  if(a>1e-6){
    float disc=b*b-4.0*a*c;
    if(disc>=0.0){
      float sq=sqrt(disc); float ta=(-b-sq)/(2.0*a), tb=(-b+sq)/(2.0*a);
      for(int i=0;i<2;i++){
        float tc=i==0?ta:tb;
        if(tc>0.001){ vec3 p=ro+rd*tc; if(abs(p.y)<=hh && tc<bestT){ bestT=tc; found=true; bestN=normalize(vec3(p.x,0.0,p.z)); } }
      }
    }
  }
  for(int s=-1;s<=1;s+=2){
    ...
    if(abs(rd.y)>1e-6){
      float tc=(fy-ro.y)/rd.y;
      if(tc>0.001){ vec3 p=ro+rd*tc; if(p.x*p.x+p.z*p.z<=r*r && tc<bestT){ bestT=tc; found=true; bestN=vec3(0.0,float(s),0.0); } }
    }
  }
  if(found){t=bestT; nl=bestN;} return found;
}
bool intersectCone(vec3 ro, vec3 rd, float r, float h, out float t, out vec3 nl){
  float k=r/h; vec3 o=ro-vec3(0.0,h*0.5,0.0);
  float a=rd.x*rd.x+rd.z*rd.z-k*k*rd.y*rd.y;
  float b=2.0*(o.x*rd.x+o.z*rd.z)+2.0*k*k*rd.y*(-o.y);
  float c=o.x*o.x+o.z*o.z-k*k*o.y*o.y;
  float bestT=1e9; bool found=false; vec3 bestN;
  if(abs(a)>1e-6){
    ...
    if(disc>=0.0){
      ...
      for(int i=0;i<2;i++){
        float tc=(-b+(i==0?-sq:sq))/(2.0*a);
        if(tc>0.001){ vec3 p=ro+rd*tc; float py=p.y+h*0.5;
          if(py>=0.0&&py<=h&&tc<bestT){ bestT=tc; found=true; vec3 side=normalize(vec3(p.x,0.0,p.z)); bestN=normalize(vec3(side.x,k,side.z)); } }
      }
    }
  }
  if(abs(rd.y)>1e-6){
    float fy=-h*0.5, tc=(fy-ro.y)/rd.y;
    if(tc>0.001){ vec3 p=ro+rd*tc; if(p.x*p.x+p.z*p.z<=r*r&&tc<bestT){ bestT=tc; found=true; bestN=vec3(0.0,-1.0,0.0); } }
  }
  if(found){t=bestT; nl=bestN;} return found;
}
bool intersectTorus(vec3 ro, vec3 rd, float R, float r, out float t, out vec3 nl){
  vec3 p=ro; bool hit=false; float totalT=0.0;
  for(int i=0;i<48;i++){
    vec2 q=vec2(length(p.xz)-R, p.y); float d=length(q)-r;
    if(d<0.001){hit=true;break;} if(totalT>40.0) break;
    totalT+=d; p=ro+rd*totalT;
  }
  if(!hit) return false;
  t=totalT; vec3 pos=ro+rd*t; vec3 cC=normalize(vec3(pos.x,0.0,pos.z))*R;
  nl=normalize(pos-cC); return true;
}
...
void main(){
  ...
  vec3 col = albedo*0.18; // إضاءة محيطة بسيطة
  for(int i=0;i<MAX_LIGHTS;i++){
    if(i>=uLightCount) break;
    vec3 ldir; vec3 radiance;
    if(uLightType[i]==1){
      ldir = normalize(-uLightDir[i]);
      radiance = uLightColor[i]*uLightIntensity[i]*0.3;
    } else {
      vec3 toL = uLightPos[i]-h.pos;
      float dist=length(toL); ldir=toL/max(dist,0.0001);
      radiance = uLightColor[i]*uLightIntensity[i]*0.3/max(dist*dist,0.05);
    }
    float ndl = max(dot(nrm,ldir),0.0);
    vec3 hVec = normalize(ldir+viewDir);
    float spec = pow(max(dot(nrm,hVec),0.0), mix(64.0,8.0,uMatRoughness[h.id]));
    col += albedo*radiance*ndl + radiance*spec*uMatMetallic[h.id]*0.5;
  }

  // خطوط شبكية بسيطة (fresnel rim) لإظهار الشكل بوضوح دون إضاءة كاملة
  float rim = pow(1.0-max(dot(nrm,viewDir),0.0), 3.0);
  col += vec3(rim*0.08);

  // Outline أزرق للعنصر المحدد
  if(h.id==uSelectedIdx){
    float edge = pow(1.0-max(dot(nrm,viewDir),0.0), 1.5);
    col = mix(col, vec3(0.35,0.55,1.0), edge*0.55);
  }
...
// شيدر العرض النهائي (tone mapping + denoise + bloom)
  ...
  if(uBloom==1){
    ...
    for(int dx=-4; dx<=4; dx+=2){
      for(int dy=-4; dy<=4; dy+=2){
        ...
        float bright = max(lum - uBloomThreshold, 0.0);
        float w = exp(-(float(dx*dx+dy*dy))/24.0);
        bloomSum += samp*bright*w;
        bloomW += w;
      }
    }
    vec3 bloomColor = bloomSum/max(bloomW,0.0001);
...
class PathTracerEngine{
  ...
  // بناء قائمة البلاطات مرتبة من المركز إلى الأطراف (حلزونياً بالمسافة)
  buildTileQueue(){
    this.tileQueue = [];
  ...
  setUniforms(prog, scene, tileRect, activeCam){
    ...
    objs.forEach((o,i)=>{
      ...
      const sssColorLoc = gl.getUniformLocation(prog,`uMatSubsurfaceColor[${i}]`);
      if(sssColorLoc) gl.uniform3f(sssColorLoc, ...(m.subsurfaceColor||[1,0.2,0.2]));
    });
    ...
    lights.forEach((l,i)=>{
      ...
      const spotCosLoc = gl.getUniformLocation(prog,`uLightSpotCos[${i}]`);
      if(spotCosLoc) gl.uniform1f(spotCosLoc, Math.cos((l.light.spotAngle||35)*Math.PI/180));
      const spotBlendLoc = gl.getUniformLocation(prog,`uLightSpotBlendCos[${i}]`);
      if(spotBlendLoc){
        const outerAngle = (l.light.spotAngle||35)*Math.PI/180;
        const blend = (l.light.spotBlend!==undefined?l.light.spotBlend:0.25);
        const innerAngle = outerAngle*(1.0-blend);
        gl.uniform1f(spotBlendLoc, Math.cos(outerAngle));
        gl.uniform1f(spotCosLoc, Math.cos(innerAngle));
      }
    });
  }
  ...
  // معاينة سريعة (rasterizer تقريبي) - إطار واحد فوري بدون تراكم، يُعرض مباشرة
  ...
  present(denoise, denoiseStrength, bloom, bloomStrength, bloomThreshold){
    ...
    gl.uniform1f(gl.getUniformLocation(this.progDisplay,'uBloomThreshold'), bloomThreshold!==undefined?bloomThreshold:1.0);
    gl.bindVertexArray(this.vao);
    gl.drawArrays(gl.TRIANGLE_STRIP,0,4);
  }
}
