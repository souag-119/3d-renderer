...
class Material{
  constructor(name){
    ...
    // نوع صلب
    this.metallic = 0.0;
    this.roughness = 0.5; // 0..1 داخلياً (يُعرض كنسبة 0-100 بالواجهة)
    this.smoothShade = 1.0; // 0 = flat shading كامل، 1 = smooth كامل
    this.subsurface = 0.0;
    this.emission = 0.0;
...
class SceneObject{
  constructor(kind, subtype){
    ...
    if(kind==='mesh'){
      ...
      this.boneParentId = null; // id عظمة الأب (لرسم الخط الأرجواني بينهما)
      this.boneChildren = [];   // ids العظام الأبناء
    }
  }
